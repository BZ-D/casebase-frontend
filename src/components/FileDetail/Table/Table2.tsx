...
const Table2: React.FC<IProp> = (props) => {
        ...
        <tr>
          ...
          <td>
            { fileDetails.litigants.map((litigant: any, i: number) => 
              <span key={i}>{ litigant.chargeMan ? '' : filterEmptyStr(litigant.unit) + ' ' }</span>
            )}
          </td>
        </tr>
        <tr>
          ...
          <td colSpan={3}>
            { fileDetails.litigants.map((litigant: any, i: number) => 
              <span key={i}>{ litigant.chargeMan ? '' : filterEmptyStr(litigant.name) + ' ' }</span>
            )}
          </td>
        </tr>
        <tr>
          ...
          <td colSpan={2}>
            { fileDetails.litigants.map((litigant: any, i: number) => 
              <span key={i}>{ litigant.unit ? '' : filterEmptyStr(litigant.name) + ' ' }</span>
            )}
          </td>
        </tr>
        <tr>
          ...
          <td colSpan={2}>
            { fileDetails.litigants.map((litigant: any, i: number) => 
              <span key={i}>{ litigant.unit ? '' : filterEmptyStr(litigant.chargeMan) + ' ' }</span>
            )}
          </td>
        </tr>
        <tr>
