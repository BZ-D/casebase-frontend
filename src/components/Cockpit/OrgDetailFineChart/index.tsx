...
import { getDetailOrganFineAPI } from "../../../api/cockpit";
...
const OrgDetailFineChart: React.FC = () => {
  ...
  // 初始化数据
  useEffect(() => {
    getDetailOrganFineAPI({ year, month }).then(res => {
      setData(res);
      setLoaded(true);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const pie = document.getElementById('organ-detail-fine-pie') as HTMLElement;
    const bar = document.getElementById('organ-detail-fine-bar') as HTMLElement;
  ...
  return (
    <>
      <div className="organ-detail-fine-chart-area">
        <div className="organ-detail-fine-chart-title">
          {
            `${year}年${month === 0 ? '全年' : month + '月'}各具体机构罚金数额 Top10`
          }
        </div>
        { loaded &&
        ...
        { loaded && 
          <Row className="organ-detail-fine-chart">
            <Col span={10}>
              <div id="organ-detail-fine-pie" />
            </Col>
            <Col span={14}>
              <div id="organ-detail-fine-bar" />
            </Col>
          </Row>
        }
      </div>
...
export default OrgDetailFineChart;