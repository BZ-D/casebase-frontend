...
import { getOrganNumAPI } from "../../../api/cockpit";
...
const OrgNumChart: React.FC = () => {
  ...
  // 初始化数据
  useEffect(() => {
    getOrganNumAPI({ year, month }).then(res => {
      setData(res);
      setLoaded(true);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const pie = document.getElementById('organ-num-pie') as HTMLElement;
    const bar = document.getElementById('organ-num-bar') as HTMLElement;
  ...
  return (
    <>
      <div className="organ-num-chart-area">
        <div className="organ-num-chart-title">
          {
        ...
        { loaded && 
          <Row className="organ-num-chart">
            <Col span={10}>
              <div id="organ-num-pie" />
            </Col>
            <Col span={14}>
              <div id="organ-num-bar" />
            </Col>
          </Row>
        }
      </div>
